...
DISPLAY_GROUP_BY_OUTLET = os.getenv("DISPLAY_GROUP_BY_OUTLET", "0") == "1"
...
[구조화 점수 규칙]
- 중요도(importance_1to5)는 아래 4개 세부점수를 근거로 일관되게 산정한다.
- market_impact_1to5: 자산가격/업종/시장수급에 미치는 파급력
- urgency_1to5: 시장이 당장 반응할 가능성과 시급성
- duration_1to5: 영향 지속기간(단기 노이즈 vs 중기/장기 추세)
- confidence_1to5: 기사 사실관계와 신뢰도, 해석의 확신도
- score_reason은 1문장으로 점수 산정 근거를 요약한다.
...
SECTOR_HINT_RULES = [
    ...
    ("경제종합", ["연준", "fed", "금리", "물가", "cpi", "ppi", "고용", "실업", "채권", "국채", "환율", "달러", "boj", "ecb", "성장률", "재정", "관세", "정책"]),
...
def clamp_score(value, default=3):
    try:
        v = int(value)
...
def normalize_meta(meta):
    raw_sector = (meta.get("sector") or "").strip()
    normalized_sector = SECTOR_ALIAS_MAP.get(raw_sector, raw_sector)
    if not normalized_sector:
        normalized_sector = "경제종합"

    breakdown = meta.get("score_breakdown", {}) or {}
    market_impact = clamp_score(breakdown.get("market_impact_1to5"), default=3)
    urgency = clamp_score(breakdown.get("urgency_1to5"), default=3)
    duration = clamp_score(breakdown.get("duration_1to5"), default=3)
    confidence = clamp_score(breakdown.get("confidence_1to5"), default=3)

    # 투자 활용도를 높이기 위해 시장파급력/시급성에 더 높은 가중치를 둔다.
    weighted = (
        market_impact * 0.40
        + urgency * 0.25
        + duration * 0.20
    ...
    calculated_importance = clamp_score(round(weighted), default=3)

    raw_importance = clamp_score(meta.get("importance_1to5"), default=calculated_importance)
    # 모델 점수와 구조화 점수가 크게 어긋나면 구조화 점수를 우선한다.
    if abs(raw_importance - calculated_importance) >= 2:
        final_importance = calculated_importance
    else:
        final_importance = raw_importance

    normalized = dict(meta)
    normalized["sector"] = normalized_sector
    normalized["importance_1to5"] = final_importance
    normalized["score_breakdown"] = {
        "market_impact_1to5": market_impact,
        "urgency_1to5": urgency,
        "duration_1to5": duration,
        "confidence_1to5": confidence,
    }
    normalized["score_reason"] = (meta.get("score_reason") or "").strip()
    return normalized
...
def build_grouped_section(section_title, enriched_list, key_name):
    ...
    if key_name == "sector":
        ordered_keys = [key for key in SECTOR_DISPLAY_ORDER if key in groups]
        remaining_keys = [key for key in groups.keys() if key not in ordered_keys]
        group_keys = ordered_keys + sorted(remaining_keys)
    else:
...
def build_flat_section(items):
    ...
    for idx, item in enumerate(items, start=1):
        ...
        if item.get("score_reason"):
            lines.append(f"· 점수근거: {item['score_reason']}")
        score_breakdown = item.get("score_breakdown", {}) or {}
        if score_breakdown:
            lines.append(
                "· 점수세부: "
                f"영향 {score_breakdown.get('market_impact_1to5', '-')}/5, "
                f"시급 {score_breakdown.get('urgency_1to5', '-')}/5, "
                f"지속 {score_breakdown.get('duration_1to5', '-')}/5, "
                f"확신 {score_breakdown.get('confidence_1to5', '-')}/5"
            )
        lines.append(f"· 링크: {item['url']}")
        lines.append("-" * 24)
        lines.append("")
...
def main():
    ...
    if not items:
        ...
        if PROMPT_ONLY or LLM_PREVIEW_ONLY:
            print("PROMPT/미리보기 모드: 외부 전송 없이 종료합니다.")
            return
